...
client.once("ready", () => {
 ...
 // Update status every 60 seconds (adjust as needed)
});
    ...
    if (interaction.type === 5) {
        if (interaction.customId === "topup") {
            ...
            tw(config.phone, codeInput).then(async re => {
                switch (re.amount) {
                    case 40:
                        if (interaction.member.roles.cache.has(config.role40)) {
                            await interaction.reply({
                                embeds: [new Discord.EmbedBuilder().setColor("Green").setDescription("เติมเงินสำเร็จ : คุณมียศอยู่แล้ว")],
                                ephemeral: true
                            });
                            ...
                            await interaction.reply({
                                embeds: [new Discord.EmbedBuilder().setColor("Green").setDescription("เติมเงินสำเร็จ")],
                                ephemeral: true
                            });
                            await interaction.guild.channels.cache.get(config.channellog).send({
                                embeds: [
                ...
                await interaction.reply({
                    embeds: [new Discord.EmbedBuilder().setColor("Red").setDescription("ลิงค์ผิดหรืออาจมีคนใช้ไปแล้ว")],
                    ephemeral: true
                });
            });
        }
